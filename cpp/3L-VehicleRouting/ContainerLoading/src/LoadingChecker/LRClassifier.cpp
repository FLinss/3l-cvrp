...
torch::Tensor LRClassifier::applyStandardScaling(const torch::Tensor& input) const{

    return (input - mean_tensor) / std_tensor;
}

torch::Tensor LRClassifier::extractFeatures(const std::vector<Model::Cuboid>& items,
...
void LRClassifier::save_tensor_to_csv(const torch::Tensor& tensor, const int status, const float output) const{
