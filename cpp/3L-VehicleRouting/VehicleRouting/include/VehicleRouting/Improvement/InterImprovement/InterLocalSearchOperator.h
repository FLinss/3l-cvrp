...
class InterLocalSearchOperator : public LocalSearchOperatorBase
{
  ...
  protected:
    std::optional<double> GetBestMove(const Model::Instance* const instance,
                                      const VRP_InputParameters* const inputParameters,
                                      ContainerLoading::BaseLoadingChecker* loadingChecker,
                                      const Helper::Timer* const mTimer,
    ...
    virtual std::vector<InterMove> DetermineMoves(const Model::Instance* instance,
                                                  const std::vector<Model::Route>& routes) const = 0;

    virtual void ChangeRoutes(std::vector<Model::Route>& routes, const InterMove& move) const = 0;
    virtual void RevertChangeRoutes(std::vector<Model::Route>& routes, const InterMove& move) const = 0; 

    void UpdateRouteVolumeWeight(std::vector<Model::Route>& routes, const InterMove& move) const;
