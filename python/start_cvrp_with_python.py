...
output_folder = r"H:\Data\ValidationData\original_data/"
input_folder = os.path.join(directory_path,"data/input/3l-cvrp/gendreau/")
parameter_files = [os.path.join(directory_path,"data/input/3l-cvrp/parameters/Parameteres_NoClassifier.json")]
                   #os.path.join(directory_path,"data/input/3l-cvrp/parameters/Parameteres_Filter.json"),
...
def add_tasks(task_queue, folder_path=input_folder):
    ...
    for seed_offset in range(5):  # seeds 0, 1, 2
...
def run_Heuristic_exe(filename, parameter_file, seed_offset, counter, lock):
    ...
    try:
        ...
        subprocess.run(command, check=True, capture_output=True, cwd=directory_path, env=env, timeout=MAX_TIME)
